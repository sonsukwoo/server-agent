...
- is_followup이 true면 기본적으로 이전 쿼리를 기반으로 작업하되,
  질문 대상이 확장되어 필요한 테이블이 달라지면 테이블 보강 재검색이 가능하다.
...
- 기본적으로 이전 쿼리에 LIMIT, ORDER BY, WHERE 조건을 추가/수정하라.
- 단, 질문 대상이 확장되어 기존 테이블만으로 답할 수 없으면 필요한 테이블 보강을 허용한다.
- **시간 범위 상속 (매우 중요)**:
...
(시간 모드가 'explicit'이고 시작 시각이 '처음'이면, 시간 하한 없이 종료 시각 이하 조건만 적용한다.)
메트릭: {metric}
조건: {condition}
...
8) **시간 모드(time_mode) 검증**:
   ...
   - 단, `time_mode='explicit'`에서 시작값이 `처음`인 경우, 하한 없이 종료 시각 이하 조건만 있어도 정상이다.
